classdef turbine_type < handle
    ...
    methods
        function obj = turbine_type(Type) % turbine_type(rotorRadius, genEfficiency, hubHeight, pP, cpctMapFunc, allowableControlMethods, description)
            %TURBINE_TYPE Construct an instance of this class
            %   The turbine characters are saved as properties
            
            Folder = sprintf('./Settings/Wind Farm/%s/',Type); % define subfolder for current wind turbine
            
            Spec = readstruct([Folder,'/Spec.txt']); % Turbine specification
            obj.description = Spec.Description;
            obj.rotorRadius = Spec.RotorDiameter/2;
            obj.rotorArea = pi*obj.rotorRadius.^2;
            obj.genEfficiency = Spec.DrivetrainEfficiency;
            obj.ratedPower = Spec.Pr;
            
            PowerYaw = readstruct([Folder,'/PowerYaw.txt']);
            obj.pP = PowerYaw.pp;

            obj.controlMethod = nan; % is set in PerfCurves
            obj.cpctMapFunc = @(x)perf_curves(x,Type); % anonymous function with placeholder for controlMethod
            obj.allowableControlMethods = {'greedy'};
