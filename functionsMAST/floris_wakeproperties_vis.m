% Calculate wake location delta Y: Eq. 8-12
factor       = (2.0*model.kd*deltax/turb.rotorDiameter)+1.0;
displacement = (wakeAngleInit(turbi)*(15.0*(factor^4.0)+(wakeAngleInit(turbi)^2.0))/ ...
               ((30.0*model.kd*(factor^5.0))/turb.rotorDiameter))- ...
               (wakeAngleInit(turbi)*turb.rotorDiameter*(15.0+(wakeAngleInit(turbi)^2.0))/(30.0*model.kd));

% Turbine #row's wake centerline location Y at the wind-farm row #column
wake_locY_wf_Vis(turbi,sample_x) = wt_locations_wf(turbi,2) + ...                    % wake location starting point
                                   model.initialWakeDisplacement + ...              % initial wake displacement
                                   (1-model.useWakeAngle) * deltax * model.bd + ... % rotation-induced lateral offset
                                   displacement;                                    % yaw-induced wake displacement
    
% Calculate overlap areas of wake on downwind samples
for sample_y = 1:length(vis.y)
   wake_dY_Vis(turbi,sample_x,sample_y) = abs(wake_locY_wf_Vis(turbi,sample_x)-vis.y(sample_y));
   wakeOverlapRelVis(turbi,sample_x,sample_y,1:3) = 0;
   for zone = 1:3
       % Calculate wake diameter for zones 1-3: Eq. 13
       wakeDiametersVis(turbi,sample_x,zone) = wakeDiameter0(turbi) + 2*ke(turbi)*model.me(zone)*deltax;
       if 2*wake_dY_Vis(turbi,sample_x,sample_y) < wakeDiametersVis(turbi,sample_x,zone)
            wakeOverlapRelVis(turbi,sample_x,sample_y,zone) = 1;
            break;
       end;
   end;
end;
