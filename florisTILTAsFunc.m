function [ flowFieldOut,windspeeds ] = florisTILTAsFunc( FFres, LocIF, yawAngles_wf, siteU, siteV, p )
addpath functionsTILT
timer.script = tic;
%% Script settings
plotLayout    = p(1);     % plot farm layout w.r.t. inertial and wind frame
plot2DFlowfield = p(2); % 2DflowFieldvisualisation in wind-aligned frame
...
if (plot2DFlowfield || plot3DFlowfield)
   ...
   flowField.resx   = FFres;     % resolution in x-axis in meters (windframe)
   flowField.resy   = FFres;     % resolution in y-axis in meters (windframe)
...
site.uInfIf   = siteU;        % x-direction flow speed inertial frame (m/s)
site.vInfIf   = siteV;        % y-direction flow speed inertial frame (m/s)
...
if plot2DFlowfield
    figure;
    flowFieldOut = plot_2d_field( flowField,site,turbines,turbType );
end;
windspeeds = [turbines.windSpeed];
