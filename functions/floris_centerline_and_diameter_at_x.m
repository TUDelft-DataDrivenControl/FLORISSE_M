function [ wake ] = floris_centerline_and_diameter_at_x( rotorDiameter,model,turbine,wake )
    ...
    for sample_x = 1:length(wake.xSamples)
        deltax = wake.xSamples(sample_x)-turbine.LocWF(1);
        if deltax >= 0
            ...
            wake.centerline(sample_x) = turbine.LocWF(2) + displacement + ...                   % initial position + yaw induced offset
