...
for i = 1:length(yawRange)
    yaw = yawRange(i);
    filename_piso = ['W:\OpenFOAM\bmdoekemeijer-2.4.0\simulationCases\RE2019\1turb_runs\piso_1turb_yaw' num2str(270-yaw) '\postProcessing\turbineOutput\0\generatorPower'];
    filename_wps = ['W:\OpenFOAM\bmdoekemeijer-2.4.0\simulationCases\RE2019\1turb_runs\wps_1turb_lowTI_yaw' num2str(270-yaw) '\postProcessing\turbineOutput\20000\generatorPower'];
    ...
    PAvg_wps(i) = mean(data_wps(time_wps>600)) / 1.225; % Correction from SOWFA
end

%% Determine FLORIS power
addpath(genpath('../../FLORISSE_M'))
pisoDB = load('10MW_piso_1turb_yaw0.mat');
wpsDB  = load('10MW_wps_1turb_lowTI_yaw0.mat');
...
controlSet_piso = control_set(layout_piso, 'yaw');
controlSet_wps  = control_set(layout_wps,  'yaw');
...
% % hold on
% greedyWs = DB.wind;
% greedyCp = squeeze(DB.mean_Cp(:,1,9))';
% greedyCt = squeeze(DB.mean_Ct(:,1,9))';
% rho = 1.225;
% WS = 7.0;
% Cp = interp1(greedyWs,greedyCp,WS);
% eta_p = 1.05;
% P = 0.5*rho*(0.25*pi*178.3^2)*WS^3.0*Cp*eta_p*cosd(yawRange).^1.45;
% hold on;
% plot(yawRange,P);
